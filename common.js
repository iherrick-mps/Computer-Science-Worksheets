...
/* Load the full JSON (paragraph + questions) for a list of manifest entries,
   in the same order as the input list. Used by pages that need to search or
   print the actual worksheet content rather than just the manifest summary. */
async function loadAllWorksheets(entries) {
  return Promise.all(entries.map(loadWorksheet));
}
